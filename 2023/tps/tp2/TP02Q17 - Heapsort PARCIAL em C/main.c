...
#include <stdlib.h>
#include <time.h>
#define NUM_JOGADORES_A_EXIBIR 11
...
void bubbleSort(Jogador *vetor, int n, int *comp, int *mov)
{
    ...
    for (int i = 0; i < n - 1; i++)
    {
        ...
        {
            (*comp)++;
            if (vetor[j].altura > vetor[j + 1].altura)
            {
...
int compararJogadores(const void *a, const void *b)
{
    ...
    if (jogadorA->altura == jogadorB->altura)
    {
    ...
    else
    {
        ...
        return jogadorA->altura - jogadorB->altura;
    }
}
...
int main()
{
    ...
    int numJogExi = (numIDs < NUM_JOGADORES_A_EXIBIR) ? numIDs : NUM_JOGADORES_A_EXIBIR;
    for (int j = 0; j < numJogExi; j++)
    {
