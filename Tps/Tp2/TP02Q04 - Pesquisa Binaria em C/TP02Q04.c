...
void criaArqLog() {
    FILE* arqLog;
    arqLog = fopen("matricula_binaria.txt", "w");
...
bool pesqBinaria(Personagem *vetor,int tamanho, char nome[])
{
    bool resp=false;
    int esq = 0, dir = tamanho-1, meio;
     ...
     while (esq <= dir){
         meio = (esq + dir) / 2;
         Comp++;
         if(strcmp(vetor[meio].name,nome) == 0){
            resp = true;
            esq = dir + 1;
         } else {
            Comp++;
  ...
  Personagem vetor[27];
    ...
    for(int i=0;i<max-1;i++){
        ...
        for(int x=i+1;x<max;x++){
            Comp++;
            if(strcmp(vetor[x].name,vetor[menor].name)<0){
                menor=x; 
            }
        }        
