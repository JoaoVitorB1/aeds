...
void criaArqLog() {
    FILE* arqLog;
    arqLog = fopen("matricula_shellsort.txt", "w");
...
void shellSort(Personagem *vetor, int n) {
    int j;
    for (int gap = n / 2; gap > 0; gap /= 2) {
        for (int i = gap; i < n; i++) {
            Personagem temp = vetor[i];
            for (j = i; j >= gap && (strcmp(vetor[j - gap].eyeColour, temp.eyeColour) > 0 || 
                (strcmp(vetor[j - gap].eyeColour, temp.eyeColour) == 0 && strcmp(vetor[j - gap].name, temp.name) > 0)); j -= gap) {
                vetor[j] = vetor[j - gap];
            }
            vetor[j] = temp;
        }
