...
void criaArqLog() {
    FILE* arqLog;
    arqLog = fopen("matricula_selecaoRecursiva.txt", "w");
...
void selectionSort(Personagem *vetor,int n, int index){
    if(index==n){
        return;
    }
    int k=index;
    for(int j=index+1;j<n;j++){
        Comp++;
        if(strcmp(vetor[j].name,vetor[k].name)<0){
            k=j;
        }
    }
    Personagem temp=vetor[k];
    vetor[k]=vetor[index];
    vetor[index]=temp;
