...
void criaArqLog() {
    FILE* arqLog;
    arqLog = fopen("matricula_avl.txt", "w");
...
void getFator(No **i, int *alturaEsq, int *alturaDir) {
    if (*i != NULL) {

        //variaveis altura direita e esquerda
        int alturaEsqSubArvore = 0;
        int alturaDirSubArvore = 0;
        
        //chamar i->esq enquanto i existe
        getFator(&((*i)->esq), &alturaEsqSubArvore, &alturaDirSubArvore);
        *alturaEsq = alturaEsqSubArvore + 1;
        
        alturaEsqSubArvore = 0;
        alturaDirSubArvore = 0;

        //chamar i->dir enquanto i existe
        getFator(&((*i)->dir), &alturaEsqSubArvore, &alturaDirSubArvore);
        *alturaDir = alturaDirSubArvore + 1;
        
        (*i)->fator = *alturaDir - *alturaEsq;
    }
    else {
        *alturaEsq = 0;
        *alturaDir = 0;
...
void inserirRec(Personagem x, No **i)
{
    ...
    {
        comp++;
        if (strcmp(x.name, (*i)->elemento.name) < 0)
        {
            inserirRec(x, &((*i)->esq));
        }
        else
        {
            ...
            {
                printf("Personagem ja existe!");
                exit(1);
            }
        }
    }
    getFator(i, &alturaEsq, &alturaDir);
    comp++;
    if ((*i)->fator == 2)
    {
        getFator(&((*i)->dir), &alturaEsq, &alturaDir);
        comp++;
        if ((*i)->dir->fator >= 0)
        {
            *i = rotacionarEsq(i);
    ...
    else
    {
        ...
        if ((*i)->fator == -2)
        {
            getFator(&((*i)->esq), &alturaEsq, &alturaDir);
            comp++;
            if ((*i)->esq->fator <= 0)
            {
                *i = rotacionarDir(i);
...
// Questao 3
void Questao3(Personagem personagens[])
{
