//Questoes finalizadas: 2,4,7,8,10
#include <stdio.h>
#include <string.h>
#include <stdbool.h>
#include <stdlib.h>
...
void startPilhaFlexivel () {
   topo = NULL;
}

void inserirPilhaFlexivel(Personagem x) {
...
void removerPilhaFlexivel() {
   if (topo == NULL) {
      printf("Erro ao remover!");
      exit(1);
   }

   char* resp=topo->elemento.name;
...
// Questao 10
void Questao10(Personagem personagens[]){
    startPilhaFlexivel();
    char linha[200];
    int quant=0;
    scanf(" %s",linha);
    while(strcmp(linha,"FIM")!=0){
        for(int i=0;i<405;i++){
            if(strcmp(personagens[i].id,linha)==0){
                inserirPilhaFlexivel(personagens[i]);
                i=500;
            }
        }
        scanf(" %s",linha);
    ...
    for(int i=0;i<quant;i++){
        scanf("%s",linha);
        if(linha[0]=='R'){
            removerPilhaFlexivel();
        }else if(linha[0]=='I'){
            scanf("%s",linha);
            for(int j=0;j<405;j++){
                if(strcmp(personagens[j].id,linha)==0){
                    inserirPilhaFlexivel(personagens[j]);
                    j=500;
                }
            }
        }
    }
    mostrarPilhaFlexivel();
}
int main(){ 
    Personagem personagens[405];
    preencherVetor(personagens);
    Questao10(personagens);
    return 0;
}