...
void criaArqLog() {
    FILE* arqLog;
    arqLog = fopen("matricula_radixsort.txt", "w");
...
void radcountingSort(Personagem *vetor,int n, int exp){
    Personagem output[n];
    int count[128]={0};

    for(int i=0;i<n;i++){count[vetor[i].id[exp]]++;}
    for(int i=1;i<128;i++){count[i]+=count[i-1];}
    for(int i=n-1;i>=0;i--){
        output[count[vetor[i].id[exp]]-1]=vetor[i];
        count[vetor[i].id[exp]]--;
    }
    for(int i=0;i<n;i++){vetor[i]=output[i];}
}



void radixsort(Personagem *vetor,int n){
    int max=36;
    for(int i=max-1;i>=0;i--){
        radcountingSort(vetor,n,i);
    }    
}
