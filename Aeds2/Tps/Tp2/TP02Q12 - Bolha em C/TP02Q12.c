...
void criaArqLog() {
    FILE* arqLog;
    arqLog = fopen("matricula_bolha.txt", "w");
...
void bubbleSort(Personagem *vetor,int n){
    for (int i=0;i<n-1;i++){
        for (int j=0;j<n-i-1;j++){
            Comp++;
            if(strcmp(vetor[j].hairColor,vetor[j+1].hairColor)>0){
                Personagem temp=vetor[j];
                vetor[j]=vetor[j+1];
                vetor[j+1]=temp;
            }else {
                Comp+=2;
                if((strcmp(vetor[j].hairColor,vetor[j+1].hairColor)==0)&&(strcmp(vetor[j].name,vetor[j+1].name)>0)){
                    Personagem temp=vetor[j];
                    vetor[j]=vetor[j+1];
                    vetor[j+1]=temp;
                }
