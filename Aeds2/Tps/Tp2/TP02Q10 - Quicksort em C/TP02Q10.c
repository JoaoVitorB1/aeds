...
void criaArqLog() {
    FILE* arqLog;
    arqLog = fopen("matricula_quicksort.txt", "w");
...
int partition (Personagem *vetor, int low, int high) {
    int pivot = high;
    int i = (low - 1);

    for (int j = low; j <= high - 1; j++) {
        Comp++;
        if (strcmp(vetor[j].house, vetor[pivot].house) < 0) {
            i++;            
            ...
            Comp+=2;
            if ((strcmp(vetor[j].house, vetor[pivot].house) == 0)&&strcmp(vetor[j].name, vetor[pivot].name)<0) {
            i++;            
            Personagem temp = vetor[i];
            vetor[i]=vetor[j];
            vetor[j]=temp;  
    ...
    Personagem temp = vetor[i+1];
    vetor[i+1]=vetor[high];
    vetor[high]=temp;    
    return (i + 1);
}

void quickSort(Personagem *vetor, int low, int high) {
    if (low < high) {
